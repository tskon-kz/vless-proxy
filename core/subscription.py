...
async def _fetch_subscription(url: str) -> tuple[list[str], str]:
    try:
...
class SubscriptionManager:
    ...
    async def startup(self) -> None:
        for url in settings.SUBSCRIPTION_URLS:
            await self._storage.add_subscription(url, fetch_interval=settings.SUBSCRIPTION_FETCH_INTERVAL)
    ...
    async def refresh_all(self) -> list[FetchResult]:
        subs = await self._storage.list_subscriptions()
        return [await self.refresh(sub.id) for sub in subs]
    ...
    async def refresh(self, sub_id: int) -> FetchResult:
        ...
        links, error = await _fetch_subscription(sub.url)
        ...
        logger.info("subscription %d refreshed: %d proxies, %d removed", sub_id, len(valid_configs), len(removed_ids))
        ...
        self._proxy_manager._create_task(self._proxy_manager._check_pending_and_reorder())
        return FetchResult(url=sub.url, success=True, count=len(valid_configs))

    def _start_poller(self, sub: SubscriptionRow) -> None:
        initial_wait = (
            max(0.0, settings.SUBSCRIPTION_FETCH_INTERVAL - (time.time() - sub.last_fetch))
            if sub.last_fetch else 0.0
        )
        self._tasks[sub.id] = asyncio.create_task(self._poller(sub.id, initial_wait))

    async def _poller(self, sub_id: int, initial_wait: float) -> None:
        wait = initial_wait
        while True:
            if wait > 0:
                await asyncio.sleep(wait)
            try:
                await self.refresh(sub_id)
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                logger.error("subscription %d poller error: %s", sub_id, exc)
